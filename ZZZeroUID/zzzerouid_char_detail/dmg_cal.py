...
def to_dmg(char_dict: dict, bl_dict: Dict[str, float]) -> Dict[str, List[str]]:
    ...
    for bl in bl_dict:
        ...
        if char_dict["avatar_profession"] == 6:
            base = bl_dict[bl] * sheer_dmg * (1 + dmg_bouns) * resist * easy_dmg
        else:
            base = bl_dict[bl] * atk * (1 + dmg_bouns) * def_area * resist * easy_dmg

        result[name] = [
            "{:.2f}".format(base * (1 + crit_dmg)),
            "{:.2f}".format(base * (1 + crit_dmg) * crit_rate + base * (1 - crit_rate)),
            "{:.2f}".format(base),
        ]
